function [ekfArgs, u, robotXYtrajNoisy, measurements] = simulateArc2(robot_pose, goal_xy, maxWheelSpeed, ekfArgs)
...
measurements = zeros(length(ekfArgs.Q),length(u));
...
for i = 1:length(u)
    ...
    ekfArgs.robotPose = robotXYtrajNoisy(:,i);
    expected = feval(ekfArgs.measFunc,ekfArgs);
    measurements(:,i) = expected + ekfArgs.Q*randn(length(ekfArgs.Q),1);
end
end