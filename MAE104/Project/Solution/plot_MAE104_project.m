...
%     pose_current = [X(i) Y(i) Th(i)];
    ...
    for j = 1:size(pfArgs.meas{i},1)
        ...
        if ~beacons(beacon_number).detected % If first time detected, set info and plot red marker
            ...
            h2b(beacon_number) = plot(axisMap_occ,beacon_xy(1),beacon_xy(2),'ro','Markersize',5);
            h2t(beacon_number) = text(axisMap_occ,beacon_xy(1)+.1,beacon_xy(2),num2str(beacon_number));
        else
            beacons(beacon_number).xy_global = beacon_xy;
...
%                 beacons(beacon_number).xy_global = beacon_xy;
                ...
                h2b(beacon_number).XData = beacon_xy(1);
                h2b(beacon_number).YData = beacon_xy(2);
                h2t(beacon_number).Position = [beacon_xy(1)+.1 beacon_xy(2)];
%             end
        end
    end
